<div>
    <div class="max-w-7xl mx-auto p-6">
        <!-- Header -->
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Campaign Deployment</h1>
            <p class="text-gray-600 mt-2">Manage and deploy campaigns to websites</p>
        </div>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2">
                                <span class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full
                                    @if($deployment->status === 'successful') bg-green-100 text-green-800
                                    @elseif($deployment->status === 'in_progress') bg-yellow-100 text-yellow-800
                                    @elseif($deployment->status === 'partial') bg-yellow-100 text-yellow-800
                                    @else bg-red-100 text-red-800 @endif">
                                    ...
                                    <svg class="w-4 h-4 mr-1 text-yellow-500 animate-spin" fill="none"
                                        stroke="currentColor" viewBox="0 0 24 24">
                                        <circle cx="12" cy="12" r="10" stroke-width="4" class="opacity-25" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 2v4m0 0a8 8 0 018 8h-4" class="opacity-75" />
                                    </svg>
                                    @elseif($deployment->status === 'partial')
                            ...
                            <td class="px-4 py-2 text-sm text-gray-500">{{ $deployment->deployed_at?->format('M j, Y
                                H:i') }}</td>
                            <td class="px-4 py-2 text-sm">
                                ...
                                @endif
                                <div class="text-xs text-gray-500">{{ $deployment->metadata['verification']['message']
                                    ?? '' }}</div>
                                @else
                            ...
                            <td class="px-4 py-2">
                                @if(in_array($deployment->status, ['failed', 'partial']))
        ...
        <div class="mt-8 bg-white rounded-lg shadow">
            ...
            <div class="p-6">
                <pre
                    class="bg-gray-100 p-4 rounded-lg overflow-auto text-sm">{{ json_encode($lastDeploymentResult, JSON_PRETTY_PRINT) }}</pre>
            </div>
        </div>
        @endif
    </div>
